...
#        -value "mencoder-mpeg2ntscdvd" -variable "::MovieMaker::movieformat"
      ...
      $w.menubar.format.menu add radiobutton -label "MP4 (ffmpeg)" \
        -value "ffmpeg-mp4" -variable "::MovieMaker::movieformat"
...
proc ::MovieMaker::buildmovie {} {
  ...
  if {$usercancel == "0"} {
    ...
    switch $movieformat {
      ...
      ffmpeg-mp4 {
        ffmpegencMP4     ;# Convert frame sequence to video file using ffmpeg
      }
      jpegframes {
...
proc ::MovieMaker::ffmpegencMP4 {} {
  ...
  switch $movieformat {
    ffmpeg-mp4{
      puts "Converting targa frames to MP4 video format"
    }
  }
  ...
  puts "ffmpeg -framerate 25 -i '$mybasefilename.%05d.tga' -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p $mybasefilename.mp4"
  file delete -force $mybasefilename.mp4
  set foo [catch { ::ExecTool::exec ffmpeg -framerate $framerate -i $mybasefilename.%05d.tga -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p $mybasefilename.mp4 >&@ stdout}]
  #file rename -force $mybasefilename.mp4 /home/cb/Videos/vmd ;#Move the movie to custom folder
}
      ...
      switch $imgformat {
        ...
        rgb {
          for {set i 0} {$i < $numframes} {incr i 1} {
            ...
            set newfilename [format "%s/$prefix$basename.%05d.$newformat" $workdir $i]
            file delete -force $newfilename
            ::ExecTool::exec convert -auto-orient "$oldfilename" "$newfilename" >&@ stdout
            set statusfrac $i
            update    ;# update the Tk window, callbacks, etc
          }
        }
